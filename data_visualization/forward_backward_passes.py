...
for ii, elem in enumerate(slope):
    ...
    if start_slope < 0 < slope[ii + 1] or start_slope > 0 > slope[ii + 1]:
        if data["x"][ii + 1] > data["x"][start_slope_index]:
            forward_lines.append((start_slope_index, ii + 1))
        else:
            backward_lines.append((start_slope_index, ii + 1))
        start_slope = None
        start_slope2 = None
...
FORWARD_PASS_COLOR = '#FE53BB'
BACKWARDS_PASS_COLOR = '#34a32a'
LINE_COLOR = '#08F7FE'
...
#for line in forward_lines:
    #pitch.lines(data["x"][line[0]], data["y"][line[0]], data["x"][line[1]], data["y"][line[1]],
                ...
                #linewidth=LINEWIDTH, color=FORWARD_PASS_COLOR, comet=True, ax=ax['pitch'])
for line in backward_lines:
    pitch.lines(data["x"][line[0]], data["y"][line[0]], data["x"][line[1]], data["y"][line[1]],
               capstyle='butt',  # cut-off the line at the end-location.
               linewidth=LINEWIDTH, color=BACKWARDS_PASS_COLOR, comet=True, ax=ax['pitch'])
    
# plotting the titles and endnote
text_effects = [path_effects.Stroke(linewidth=3, foreground='black'),
                path_effects.Normal()]
ax['title'].text(0.5, 0.3, f'Backward Passes Made',
...
for i in range(1, NUM_GLOW_LINES + 1):
    ...
    #                color=FORWARD_PASS_COLOR, comet=True, ax=ax['pitch'])
    for line in backward_lines:
        pitch.lines(data["x"][line[0]], data["y"][line[0]], data["x"][line[1]], data["y"][line[1]],
                    linewidth=LINEWIDTH + (DIFF_LINEWIDTH * i),
                    capstyle='round',  # capstyle round so the glow extends past the line
                    alpha=ALPHA_PASS_LINE / NUM_GLOW_LINES,
                    color=BACKWARDS_PASS_COLOR, comet=True, ax=ax['pitch'])
